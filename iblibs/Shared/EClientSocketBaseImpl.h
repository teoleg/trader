...
{
	//fprintf(stderr,"Im am in EClientSocketBase::reqMktData\n");
	// not connected?
	if( !m_connected) {
	...
	//fprintf(stderr,"Before sending market data request\n");
									   ...
									   int useRTH, int formatDate)
{
	...
	//fprintf(stderr, "Im in EClientSocketBase::reqHistoricalData\n");
...
{
	//fprintf(stderr,"EClientSocketBase::checkMessages() - invoked, state = %d\n", m_connected);
	if( !isSocketOK())
		return false;
...
int EClientSocketBase::processConnectAck(const char*& beginPtr, const char* endPtr)
{
	...
	//fprintf(stderr,"EClientSocketBase::processConnectAck() - called\n");
	...
	try {
		...
		fprintf(stderr,"Received server verions from TWS: %d\n", m_serverVersion);
...
int EClientSocketBase::processMsg(const char*& beginPtr, const char* endPtr)
{
	...
	//fprintf(stderr,"EClientSocketBase::processMsg - called\n");
	...
	try {
		...
		//fprintf(stderr,"Message Type Received = [%d]\n", msgId);
		...
		switch( msgId) {
			case TICK_PRICE:
			{
				...
				//fprintf(stderr,"In PM: TICK_PRICE: version=[%d],ticketId=[%d],tickTypeInt=[%d],price=[%g],size=[%d],canAutoExecute=[%d]\n",
				...
				{
					//fprintf(stderr,"In PM: tickSize tralling.. \n");
					//TickType sizeTickType = NOT_SET;
				...
				//fprintf(stderr,"SZ tick=%d, size=%d\n", tickTypeInt, size);
				m_pEWrapper->tickPriceEx( tickerId, (TickType)tickTypeInt, price, canAutoExecute, sizeTickType, size);
			...
			case TICK_SIZE:
			{
				...
				//fprintf(stderr,"In PM: TICK_SIZE: version=[%d],tickerId=[%d],tickTypeInt=[%d],size=[%d]\n",
			...
			case TICK_OPTION_COMPUTATION:
			{
				...
				//fprintf(stderr,"In PM: TICK_OPTION_COMPUTATION: ver=[%d],tickerId=[%d],tickTypeInt=[%d],impliedVol=[%g],delta=[%g],optPrice=%g],pvDividend=[%g],gamma=[%g],vega=[%g],theta=[%g],undPrice=[%g]\n",
				//		version,tickerId,tickTypeInt,impliedVol,delta,optPrice,pvDividend,gamma,vega,theta,undPrice); 
			...
			case TICK_GENERIC:
			{
				...
				//fprintf(stderr,"In PM: TICK_GENERIC: ver=[%d],tickerId=[%d],tickTypeInt=[%d],value=[%g]\n",
			...
			case TICK_STRING:
			{
				...
				//fprintf(stderr,"In PM: TICK_STRING: ver=[%d],tickerId=[%d],tickTypeInt=[%d],value=[%s]\n",
				//		version,tickerId,tickTypeInt,value.c_str());
			...
			case TICK_EFP:
			{
				...
				/*
				fprintf(stderr,"In PM: TICK_EFP: ver=[%d],tickerId=[%ld],tickTypeInt=[%d],basisPoints=[%d],formattedBasisPoints=[%s],impliedFuturesPrice=[%g],holdDays=[%d],futureExpiry=[%g],dividendImpact=[%g],dividendsToExpiry=[%g]\n",
					version,
					tickerId,
					tickTypeInt,
					basisPoints,
			...
			case ORDER_STATUS:
			{
				...
				//fprintf(stderr,"In PM: ORDER_STATUS: ver=[%d],orderId=[%d],status=[%s],filled=[%d],remaining=[%d],avgFillPrice=[%g],permId=[%d],parentId=[%d],lastFillPrice=[%g],clientId=[%d],whyHeld=[%s]\n",
				//	version,
				//	orderId,
				//	status.c_str(),
				//	filled,
			...
			case ERR_MSG:
			{
				...
				//fprintf(stderr,"In PM: ERR_MSG: ver=[%d],id=[%d],errorCode=[%d],errorMsg=[%s]\n",
				//		version,id,errorCode,errorMsg.c_str());
			...
			case NEXT_VALID_ID:
			{
				...
				//fprintf(stderr,"In PM: NEXT_VALID_ID: version=[%d], orderId=[%d]\n",version,orderId);
			...
			case MANAGED_ACCTS:
			{
				...
				//fprintf(stderr,"In PM: MANAGED_ACCTS: version=[%d], list=[%s]\n", version, accountsList.c_str());
			...
			case HISTORICAL_DATA:
			{
				...
				//fprintf(stderr,"Received HISTORICAL_DATA from Server...");
				...
				for( int ctr = 0; ctr < itemCount; ++ctr) {
					...
					//fprintf(stderr,"HIST DATA: open[%g],high[%g],low[%g],close[%g],volume[%d]\n",
